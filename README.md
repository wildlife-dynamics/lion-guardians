...
This repository contains custom tasks and workflows for lion-guardians. Follow the setup steps below to get started.


## Project Structure

- `src/ecoscope-workflows-ext-lion-guardians`: Contains custom tasks
...
## Task Development
The tasks are defined under `src/ecoscope-workflows-ext-lion-guardians/ecoscope_workflows_ext_lion_guardians/tasks`. Here's an example of a task
...
You can also add other dependencies in `src/ecoscope-workflows-ext-lion-guardians/pyproject.toml` under [tool.pixi.dependencies] section

Unittests are under `src/ecoscope-workflows-ext-lion-guardians/tests` and can be tested it by
```bash
pixi run pytest-310
```
...
2. Run the following command to compile your workflow:
   ```bash
   pixi run compile-guardians
   ```
   
   This will generate a folder called `ecoscope-workflows-guardians-workflow` with your compiled workflow.

   Later on if you update the workflow spec you can recompile it using:
   ```bash
   pixi run recompile-guardians
   ```

3. Test your workflow. First set up your output directory by
   ```bash
   mkdir -p /tmp/workflows/lion-guardians/guardians/output
   export ECOSCOPE_WORKFLOWS_RESULTS=file:///tmp/workflows/lion-guardians/guardians/output
   ```
   Then run your workflow by
   ```bash
   cd workflows/guardians/ecoscope-workflows-guardians-workflow
   pixi run ecoscope-workflows-guardians-workflow run --config-file ../param.yaml --execution-mode sequential --mock-io
   ```
   You can find the results in your output folder in `/tmp/workflows/lion-guardians/guardians/output/result.json`
...
5. Update the version in pixi.toml
   ```toml
   [dependencies.ecoscope-workflows-ext-lion-guardians]
